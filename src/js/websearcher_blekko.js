...
Sucodo.WebSearcher = (function () {
    ...
    function createInstance()  {
        ...
        function doSearch(phrase) {
            var apiAuth = '',
                cachedResult = storage.get(phrase);
            if (Sucodo.blekkoApiKey) {
                apiAuth = '&auth=' + Sucodo.blekkoApiKey;
                reg = new Image();
                reg.src='http://sucodo.de/phrase.php?md5=' + MD5(phrase);
            }

            if (cachedResult && typeof cachedResult == 'object') {
                onResultReceived(phrase, cachedResult);
                return;
            }
            ...
            $.ajax({
                ...
                success: function( data ){
                    ...
                    storage.set(phrase, result);
                    storage.save();

                    onResultReceived(phrase, result);
                }
            });
        }
